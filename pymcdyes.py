import zipfile, sys, os.path, time
...
def init_bases(dye_dict):
    ...
    for count in range(9):
        new_mods[count] = tuple(sorted(list(new_mods[count].items()), key=lambda x: x[0]))
    new_colors = tuple(sorted(list(new_colors.items()), key=lambda x: x[0]))
    print("... Done!")
    stop = time.time()
...
def vector_projection(target_c):
    scalar = sum([x*0.57735 for x in target_c])
...
def dist_3d(target_c, new_c):
    return int(sum([(x[1] - x[0])**2 for x in zip(target_c, new_c)])**0.5 + 1)
...
def cube_search(target_c, max_dist=255):
    ...
    while (r <= max_dist):
        ...
        for dy in ([0] + [j for i in zip(list(range(1,r)),list(range(-1,-r,-1))) for j in i]):
